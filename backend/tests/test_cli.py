...
class TestRules:
    def test_map_finding_known_check(self):
        from chainaudit.evm_rules import map_finding
    ...
    def test_all_severity_rules_have_cvss(self):
        from chainaudit.evm_rules import RULES
    ...
    def test_slither_to_rule_mapping_complete(self):
        from chainaudit.evm_rules import SLITHER_TO_RULE, RULES
...
class TestScanner:
    def test_parse_slither_report_missing_file(self, tmp_path):
        from chainaudit.evm_scanner import parse_slither_report, SLITHER_JSON
    ...
    def test_parse_slither_report_empty_detectors(self, tmp_path):
        from chainaudit.evm_scanner import parse_slither_report, SLITHER_JSON, REPORTS_DIR
...
class TestCLI:
    def _run(self, args: list[str]):
        ...
        from chainaudit.cli import build_parser, cmd_scan
        import contextlib
    ...
    def test_help(self):
        from chainaudit.cli import build_parser
        parser = build_parser()
    ...
    def test_collect_sol_files_single(self, valid_sol):
        from chainaudit.cli import _collect_sol_files
    ...
    # -----------------------------------------------------------------------
    # Fixed: patch chainaudit.cli.route_scan instead of non-existent
    # run_slither / run_foundry_tests attributes that ChatGPT invented.
    # _scan_file() delegates entirely to route_scan(), so that is the correct
    # seam to mock.
    # -----------------------------------------------------------------------

    @patch("chainaudit.cli.route_scan")
    def test_scan_file_success(self, mock_route, valid_sol, mock_slither_findings):
        mock_route.return_value = {
            "status": "success",
            "chain": "ethereum",
            "risk_score": 75,
            "findings": mock_slither_findings,
            "total_findings": len(mock_slither_findings),
            "exploit_simulation": {"success": True, "stdout": "", "stderr": ""},
        }
        from chainaudit.cli import _scan_file
    ...
    @patch("chainaudit.cli.route_scan")
    def test_scan_file_slither_failure(self, mock_route, broken_sol):
        mock_route.return_value = {
            "status": "error",
            "chain": "ethereum",
            "risk_score": 0,
    ...
    @patch("chainaudit.cli.route_scan")
    def test_ml_only_skips_simulation(self, mock_route, valid_sol, mock_slither_findings):
        mock_route.return_value = {
            "status": "success",
            "chain": "ethereum",
            "risk_score": 75,
            "findings": mock_slither_findings,
            "total_findings": len(mock_slither_findings),
            "exploit_simulation": {"success": False, "stdout": "", "stderr": "skipped"},
        }
        from chainaudit.cli import _scan_file
        result = _scan_file(valid_sol, ml_only=True)
        # Verify route_scan was called with ml_only=True
        mock_route.assert_called_once_with(valid_sol, ml_only=True)
        assert result["exploit_simulation"]["stderr"] == "skipped"

    @patch("chainaudit.cli.route_scan")
    def test_json_output_valid(self, mock_route, valid_sol, mock_slither_findings, capsys):
        mock_route.return_value = {
            "status": "success",
            "chain": "ethereum",
            "risk_score": 75,
    ...
    @patch("chainaudit.cli.route_scan")
    def test_exit_code_critical(self, mock_route, valid_sol, mock_slither_findings):
        mock_route.return_value = {
            "status": "success",
            "chain": "ethereum",
            "risk_score": 90,
    ...
    @patch("chainaudit.cli.route_scan")
    def test_exit_code_no_critical(self, mock_route, valid_sol):
        mock_route.return_value = {
            "status": "success",
            "chain": "ethereum",
            "risk_score": 10,
            "findings": [
                {"title": "Low Issue", "severity": "LOW", "description": "...",
                 "fix": "...", "check": "events-maths", "confidence": "Medium",
                 "occurrences": 1}
            ],
            "total_findings": 1,
            "exploit_simulation": {"success": True, "stdout": "", "stderr": ""},
...
class TestMLPredictor:
    ...
    def test_predictor_no_model_file(self, tmp_path, monkeypatch):
        try:
            import chainaudit.ml.predictor as pred_module
            monkeypatch.setattr(pred_module, "MODEL_PATH", tmp_path / "nonexistent.joblib")
            from chainaudit.ml.predictor import ExploitabilityPredictor
...
class TestSolanaRules:
    def test_all_rules_have_required_fields(self):
        from chainaudit.solana_rules import SOLANA_RULES
    ...
    def test_get_rule_known(self):
        from chainaudit.solana_rules import get_rule
    ...
    def test_compute_solana_risk_score_empty(self):
        from chainaudit.solana_rules import compute_solana_risk_score
    ...
    def test_patterns_have_required_fields(self):
        from chainaudit.solana_rules import SOLANA_PATTERNS, SOLANA_RULES
        for p in SOLANA_PATTERNS:
            assert "rule_id" in p
            assert "patterns" in p
            assert "anti_patterns" in p
            rule_ids = [r.rule_id for r in SOLANA_RULES]
            assert p["rule_id"] in rule_ids, f"Pattern rule_id '{p['rule_id']}' not in SOLANA_RULES"
...
""")
    ...
    def test_is_solana_project_rs_file(self, vulnerable_rs):
        from chainaudit.solana_scanner import is_solana_project
    ...
    def test_collect_rs_files_single(self, vulnerable_rs):
        from chainaudit.solana_scanner import _collect_rs_files
    ...
    def test_pattern_scan_detects_unsafe(self, vulnerable_rs):
        from chainaudit.solana_scanner import run_pattern_scan
    ...
    def test_detect_anchor_project(self, tmp_path):
        from chainaudit.solana_scanner import detect_anchor_project
